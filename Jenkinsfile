pipeline {
    ...
    environment {
        // --- 1. Configuración del Stack (Variables Globales) ---
        ...
        // --- 3. Tags de Imagen (Exportadas al Shell) ---
    ...
    stages {
        stage('1. Calidad (SonarQube)') {
            steps {
                script {
                    ...
                    // Usamos comillas simples porque todas las variables son del bloque 'environment'
                    sh '''
                    docker run --rm \
                        --network ${STACK_NET} \
        ...
        stage('2. Integración Efímera (Tests)') {
            steps {
                script {
                    // Variables locales de Groovy (def)
                    ...
                    try {
                        ...
                        // USAMOS COMILLAS DOBLES (""") para inyectar variables locales (def)
                        // Escapamos \$i y \$((...)) para que Linux los maneje
                        sh """
                        i=1
                        while [ \$i -le 30 ]; do
        ...
        stage('3. Entrega Certificada (Harbor)') {
            steps {
                script {
                    ...
                    // Comillas simples para máxima seguridad con credenciales
                    sh '''
